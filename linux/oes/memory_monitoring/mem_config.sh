...
date=$(date)                                    # standard date/time stamp
...
echo never > /sys/kernel/mm/transparent_hugepage/enabled | tee -a ${log}
# Turn transparent_hugepage off at boot
bm=$(cat /boot/grub/menu.lst | grep transparent)
if [ -z ${bm} ]; then
	logit "Adding transparent_hugepage=never to grub boot menu so it is always disabled"
  awk '/splash/ {$0=$0" transparent_hugepage=never"} 1' /boot/grub/menu.lst > /tmp/menu.lst && mv /tmp/menu.lst /boot/grub/ | tee -a ${log}
else
...
echo 1 > /proc/sys/vm/zone_reclaim_mode | tee -a ${log}
# Enable zone_reclaim_mode
sc=$(cat /etc/sysctl.conf | grep "Virtual memory settings")
if [ -z ${sc} ]; then
	logit "Setting zone reclaim mode so it is persistent"
  echo -e "# Virtual memory settings" >> /etc/sysctl.conf | tee -a ${log}
  echo -e "vm.zone_reclaim_mode = 1" >> /etc/sysctl.conf | tee -a ${log}
else
...
if [ ${swp} != 60 ]; then
  ...
  echo 60 > /proc/sys/vm/swappiness | tee -a ${log}
  sw1=$(cat /etc/sysctl.conf | grep swappiness) | tee -a ${log}
  if [ -z ${sw1} ]; then
    echo -e "vm.swappiness = 60" >> /etc/sysctl.conf | tee -a ${log}
  else
