...
#  LAST UPDATED: Thu May 31 2018 10:17
#      REVISION: 8
#     SCRIPT ID: ---
# SSC UNIQUE ID: 28
#===============================================================================
...
if [ $ru -le 60 ]; then
  ...
  echo "The root partition is currently consuming $rup of avaialable space." | tee -a $rep
  echo "Consider cleaning up the root partition or adding space to the server to proceed." | tee -a $rep
fi
/bin/sleep 5

# check for available patches for the currnet OS and OES versions
/usr/bin/clear
echo "" | tee -a $rep
