...
# Script Name: highutilCIFS.sh 
# Description: This script will gstack the cifsd process the amount of times specified when loading the script.
#              After each gstack is taken and written the script pauses 10 seconds before grabing the next gstack. 
#              It writes gstack output information to the /tmp/novell/ directory Example:  /tmp/novell/gstack1.log - gstack5.log if you want 5 gstacks.
...
# Updated on: 08/03/2018   Reconfigured to capture gstacks of CIFS OOM issue
#                                                                
#################################################################
...
do	
	...
	# echo $JAVA_PID
	CIFS_PID=`/bin/cat $PIDDIR/cifsd.pid`
	echo $CIFS_PID
	GSTACKCOUNTER=0
	...
	do
		...
		logdate
		log "[*] Grabbing a gstack and writing to $GSTACKLOGS for the cifsd process PID# $CIFS_PID " 
		/bin/date >>$GSTACKLOGS
		log "***Start writing Top stats for java PID $CIFS_PID***"
		/usr/bin/top -b -n1 -H -p $CIFS_PID >>$GSTACKLOGS
		log "" >>$GSTACKLOGS
		log "-----------------top -H per thread %CPU of CIFS process----------------" >>$GSTACKLOGS
		/usr/bin/top -b -n1 -H -p $CIFS_PID | grep java >>$GSTACKLOGS
		log "[*] Finished writing Top stats for cifs PID $CIFS_PID successfully! "
