...
#  LAST UPDATED: Thu Nov 24 2016 08:02
#      REVISION: 7
#     SCRIPT ID: ---
# SSC UNIQUE ID: ---
#===============================================================================
...
dumpdir='/var/crash'                            # default location for kernel dumps (future)
...
if [ $mailed = 0 ]; then
  if [ -n "$newcore" -a "$email" ]; then
    mail_body | mail -s "An application core has occurred on $host" -a /root/7004526.pdf -a /root/cores.txt $email
    echo "mailed=1" > $mailc
  else
    logit "Will check again in 12 hours to see if a new application core has occurred."
    logit "---------------------------------------------------------------------------"
  fi
else
