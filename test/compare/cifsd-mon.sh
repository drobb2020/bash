...
#   LAST UDATED: Thu Mar 01 2018 13:54
#       VERSION: 0.1.4
#     SCRIPT ID: 000
# SSC SCRIPT ID: 00
#===============================================================================
...
mfrom=cifs-monitor                               # email sender
email=root                                       # email recipient(s)
log='/var/log/cifsd-mon.log'                     # log name and location (if required)
...
function mail_body() { 
echo -e "The novell-cifs daemon is not running on $host. An automatic restart is being attempted now. If this restart fails you will receive a second message.\nA application core may have been generated when novell-cifsd died, the core-checker script has been called to investigate. You will receive a separate email if a core was generated.\nPlease review the cifs log located at /var/log/cifs/cifs.log to see if a reason was logged for the daemon stopping."
}

function mailbody2() { 
echo -e "The automatic restart of the novell-cifs daemon has failed and may not be running on $host. Please log into the server and attempt a manual restart of the daemon. Follow up by checking the log (/var/log/cifs/cifs.log) for errors."
}
...
if [ $cifsdReturnCode == "0" ]; then
  ...
  if [ -n $email ]; then
    mail_body | mail -s "novell-cifs is DOWN on $host and will be restarted automatically" -r $mfrom $email
  fi
...
if [ $cifsdReturnCode == "0" ]; then
  ...
  core=$(ls /var/core/ | grep cifsd > /tmp/cifs_core_ck_$$.txt)
  if [ -s /tmp/cifs_core_ck_$$.txt ]; then
