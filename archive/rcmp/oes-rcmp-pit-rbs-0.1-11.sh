...
##############################################################################
#
#    oes-rcmp-pit-rbs.sh - RCMP Post Installation script for local new server
...
# Last updated: Thu Jun 06 15:03:26 2013 
...
echo "Turning off access timestamp and salvage on the MAIL volume."
...
if [ -z /tmp/prl0.tmp.$$ ]
	...
	echo "The setting PermitRootLogin has already been set to NO, current configuration not modified ..."
fi
...
if [ -z /tmp/p0.tmp.$$ ]
	...
	echo "The setting Port has already been set to 3479, current configuration not modified ..."
fi
...
namconfig set admin-fdn=cn=arsscadmin,o=atl
namconfig set preferred-server=$(cat /tmp/ip1.tmp.$$)
if [ -s /tmp/ip2.tmp.$$ ] && [ -s /tmp/ip3.tmp.$$ ]
    then
	namconfig set alternative-ldap-server-list=$(cat /tmp/ip2.tmp.$$),$(cat /tmp/ip3.tmp.$$)
fi
if [ -s /tmp/ip2.tmp.$$ ]
    then
	namconfig set alternative-ldap-server-list=$(cat /tmp/ip2.tmp.$$)
    else
	echo "No Alternative LDAP servers were provided, continuing without modifying this entry ..."
fi
...
SC0=$(cat /etc/snmp/snmpd.conf | grep -w syscontact | grep -v "#")
SL0=$(cat /etc/snmp/snmpd.conf | grep -w syslocation | grep -v "#")
...
echo -e rocommunity tsb_public 127.0.0.1 >> /etc/snmp/snmpd.conf
echo -e rocommunity tsb_public specnatrp.rcmp-grc.gc.ca >> /etc/snmp/snmpd.conf
echo -e rocommunity tsb_public specnatft.rcmp-grc.gc.ca >> /etc/snmp/snmpd.conf
echo -e rocommunity tsb_public ehealth1.rcmp-grc.gc.ca >> /etc/snmp/snmpd.conf
echo -e rocommunity tsb_public ehealth2.rcmp-grc.gc.ca >> /etc/snmp/snmpd.conf
echo -e trapcommunity tsb_public >> /etc/snmp/snmpd.conf
echo -e trapsink acpic-s1854.rcmp-grc.gc.ca >> /etc/snmp/snmpd.conf
echo -e trapsink adr-s1170.rcmp-grc.gc.ca >> /etc/snmp/snmpd.conf
...
# BEX Configuration
echo "Changing BEX configuration"
BEX1=/opt/bex/misc
BEX2=/opt/backexpress/misc
SSDMBS0=$(cat $DEX1 | grep -w SSDMBS)
SSDMBS1=$(cat $BEX2 | grep -w SSDMBS)
if [ -e $BEX1/environ.txt ]
    then 
	sed -i "s/$SSDMBS0/SSDMBS='--changedfiles=archivebitonly'/" $BEX1/environ.txt
    else
	if [ -e $BEX2/environ.txt ]
	    then
		sed -i "s/$SSDMBS1/SSDMBS='--changedfiles=archivebitonly'/" $BEX2/environ.txt
	fi
fi
echo "BEX configuration is complete."
sleep 2
clear

# NCP Configuration
echo "Changing NCP configuration"
