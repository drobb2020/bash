...
##############################################################################
#
#    oes-rcmp-pit-lns.sh - RCMP Post Installation script for local new server
...
# Last updated: Thu Jun 06 15:11:44 2013 
...
echo -e rwcommunity no 127.0.0.1 >> /etc/snmpd.conf
...
if [ -e $BEX1/environ.txt ]
    then 
	sed -i "s/SSDMBS=/SSDMBS='--changedfiles=archivebitonly'/" $BEX1/environ.txt
    else
	if [ -e $BEX2/environ.txt ]
	    then
		sed -i "s/SSDMBS=/SSDMBS='--changedfiles=archivebitonly'/" $BEX2/environ.txt
	fi
fi
