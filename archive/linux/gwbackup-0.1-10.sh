...
# Last updated: Fri Aug 03 14:29:42 2012 
# Crontab command: 
...
GWMTA="/var/opt/novell/groupwise/mail/excs2-do"
GWPOA="/var/opt/novell/groupwise/mail/excs2-po"
EMAIL=gwadmin@excs2net.org
HT=$HOST-$TODAY
...
ncpmount -S excs2-ott-s004 -A 192.168.0.54 -U admin.excs2 -P nashira!=000 -V BACKUP -m /media/backup
...
cp /media/backup/excs2-do/*gwbk.* /root/reports/mta-backup$(date +'%m%d').log
cp /media/backup/excs2-po/*gwbk.* /root/reports/poa-backup$(date +'%m%d').log
...
mail -s "$HOST GroupWise Backup Report" -a $REPDIR/$REPORTZIP $EMAIL < /root/bin/GW-backupmsg.txt
