...
#         USAGE: ./gms_top.sh
# 
#   DESCRIPTION: Grabs the output of top for Python when GMS is behaving slow
#
...
#  LAST UPDATED: Wed Sep 07 2016 11:14
#       VERSION: 4
#     SCRIPT ID: 074
# SSC UNIQUE ID: 38
#===============================================================================
...
uid=38                                          # SSC/RCMP script id number
...
log='/var/log/gms_top.log'                      # logging (if required)
...
ps -eaf | grep python | grep -v printer | grep -v grep | awk '{ print $2 }' > /root/gms/pids_of_python

# Record what each pid represents
ps -eaf | grep python | grep -v printer | grep -v grep | awk '{ print $1, $2, $9 }' > /root/gms/gms_pids.txt

# Do a for loop to do 5 top captures for each pid 60 seconds apart.
for (( i=1; i<=5; i++ ))
do
  # Run top for each pid in the file
  for p in $(cat /root/gms/pids_of_python)
  do
...
echo "---------------------------------------------------------------"
echo "GMS top collction is complete, you can now reboot the server."
echo "---------------------------------------------------------------"
