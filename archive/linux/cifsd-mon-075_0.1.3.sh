...
#  LAST UPDATED: Tue Dec 12 2017 11:02
#      REVISION: 3
#     SCRIPT ID: 00
#===============================================================================
version=0.1.3                               # script version
sid=00                                      # script ID number
ts=$(date +"%b %d %T")                      # general date/time stamp
host=$(hostname)                            # host name of local server
user=$(whoami)                              # user checking routine
mfrom=cifsd-monitor			    # email sender
email=ssc_cas_admin@rcmp-grc.gc.ca          # email accounts
log='/var/log/cifs/cifsd-mon.log'           # logging (if required)
...
if [ $cifsdReturnCode == "0" ]; then
  ...
  if [ $cifsdReturnCode2 = 0 ]; then
    ...
    if [ -n $email ]; then
      echo -e "The restart of the novell-cifs daemon has failed and may not be running on $host. Please attempt a manual restart, and check the log (/var/log/cifs/cifs.log) for errors." | mail -s "ATTENTION: novell-cifs restart on $host failed, please investigate!" -r $mfrom $email
    fi
  fi
fi
