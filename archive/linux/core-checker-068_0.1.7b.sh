...
#  LAST UPDATED: Thu Dec 12 2017 11:19
#      REVISION: 7
#     SCRIPT ID: ---
# SSC UNIQUE ID: ---
#===============================================================================
...
email=EAIMS_DIR@rcmp-grc.gc.ca,diradmin@rcmp-grc.gc.ca,david.robb@rcmp-grc.gc.ca                 # default email value
...
if [ $mailed = 0 ]; then
  if [ -n "$newcore" -a "$email" ]; then
    ...
    logit "Will check again in $corefreq hours to see if a new application core has occurred."
    logit "---------------------------------------------------------------------------"
  fi
else
