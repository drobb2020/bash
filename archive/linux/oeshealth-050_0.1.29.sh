...
#   LAST UDATED: Thu Mar 15 2018 11:18
#       VERSION: 0.1.29
#     SCRIPT ID: 050
# SSC SCRIPT ID: 00
#===============================================================================
version=0.1.29                                    # version number of the script
sid=050                                          # personal script ID
...
log='/var/log/oeshealth.log'                     # log name and location (if required)
...
rl=$(/sbin/runlevel | cut -f 2 -d " ")           # current runlevel
report=${repdir}/${repname}-${host}-${today}.txt # full report path and name
osver=$(cat /etc/SuSE-release | grep VERSION | cut -f 2 -d "=" | sed -e 's/^[ \t]*//') # OS version
LIST=$(chkconfig --list | grep 5:on | cut -f-1 -d" "|tr -t "\012" "\040") # Enabled services
#===============================================================================
...
# mail message
function mail_body1() { 
# move oes-healthmsg.txt contents here! Bump to rev 0.2.1 when done.
}
