...
# Last updated: Thu Jul 31 13:30:40 2014 
...
logit "cifsd with PID $P1 is currently consuming $MEMF1% of total memory"
logit "cifsd with PID $P1 is currently consuming $CPUF1% of the CPU."
logit "cifsd with PID $P2 is currently consuming $MEMF2% of total memory"
logit "cifsd with PID $P2 is currently consuming $CPUF2% of the CPU."
...
if [ $# -lt 2 ] 
	...
	if [ ${MEM1} -ge $1 ]
    	   then
		logit "Memory consumption on cifsd PID $P1 is greater than $1%, sending email to $EMAIL"
		echo -e "The $PROC daemon with pid $P1 is using $MEMF1%, this is a greater memory consumption than expected with only $CC users connected.\nPlease review the attached file and do one of the following tasks:\n1) Restart the daemon\n2) Restart the server\n3) If this is a clustered resource, fail it over to another node." | mail -s "High memory consumption on $HOST by cifsd" -a /tmp/topcifsd.txt $EMAIL
		logit "An e-mail alert has been sent to $EMAIL"
           else
		logit "Memory consupmtion for cifsd (PID $P1) is in the normal range at this time"
        fi
        if [ ${MEM2} -ge $1 ]
            then
                logit "Memory consumption on cifsd PID $P2 is greater than $1%, sending email to $EMAIL"
                echo -e "The $PROC daemon with pid $P2 is using $MEMF2%, this is a greater memory consumption than expected with only $CC users connected.\nPlease review the attached file and do one of the following tasks:\n1) Restart the daemon\n2) Restart the server\n3) If this is a clustered resource, fail it over to another node." | mail -s "High memory consumption on $HOST by cifsd" -a /tmp/topcifsd.txt $EMAIL
		logit "An e-mail alert has been sent to $EMAIL"
           else
		logit "Memory consupmtion for cifsd (PID $P2) is in the normal range at this time"
        fi
    if [ ${CPU1} -ge $2 ]
        then
	    logit "CPU consumption on cifsd PID $P1 is greater than $2%, sending email to $EMAIL"
	    echo -e "The $PROC daemon with $P1 is using $CPUF1%, this is a greater CPU utilization than expected with only $CC users connected.\nPlease review the attached file and do one of the following tasks:\n1) Restart the daemon\n2) Restart the server\n3) If this is a clustered resource, fail it over to another node." | mail -s "High CPU utilization on $HOST by cifsd" -a /tmp/topcifsd.txt $EMAIL
	    logit "An e-mail alert has been sent to $EMAIL"
        else
            logit "CPU utilization for cifsd (PID $P1) is in the normal range at this time"
    fi
    if [ ${CPU2} -ge $2 ]
        then
	    logit "CPU consumption on cifsd PID $P2 is greater than $2%, sending email to $EMAIL"
	    echo -e "The $PROC daemon with $P2 is using $CPUF2%, this is a greater CPU utilization than expected with only $CC users connected.\nPlease review the attached file and do one of the following tasks:\n1) Restart the daemon\n2) Restart the server\n3) If this is a clustered resource, fail it over to another node." | mail -s "High CPU utilization on $HOST by cifsd" -a /tmp/topcifsd.txt $EMAIL
	    logit "An e-mail alert has been sent to $EMAIL"
        else
            logit "CPU utilization for cifsd (PID $P2) is in the normal range at this time"
    fi
fi
