...
##############################################################################
#
#    oeshealth.sh - Create an automated health report for OES Linux servers
...
# Last updated: Thu May 28 07:45:22 2015 
# Suggested crontab command: 0 3 * * 1,3,5 /root/bin/oeshealth.sh #for weekly execution
...
if [[ -e $NDSCFG  && -e $NDSLOG ]]; then
  /bin/rm $NDSLOG
fi

# Create new report and set date timestamp
addspace
echo "--[ SLES & OES Health Report v${REL} ]------------------------------------------" >> $REPORT
