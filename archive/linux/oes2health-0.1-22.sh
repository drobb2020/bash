...
# Last updated: Tue Jul 30 08:12:19 2013 
# Suggested crontab command: 0 3 * * 1,3,5 /root/bin/oes2health.sh #for weekly execution
#                            * * 2 * * /root/bin/oes2health-0.1.19.sh #for monthly execution
# Supporting file: /root/bin/oes-healthmsg.txt
...
# Custom Variables
REPDIR=/root/reports
REPNAME=OES_Health_Report
NDSCFG=/etc/opt/novell/eDirectory/conf/nds.conf
NDSLOG=/var/opt/novell/eDirectory/log/ndsrepair.log
...
rep_cmd $REPORT "cat /etc/SuSE-release | grep SUSE | cut -d'(' -f1"
echo "--[ SLES Service Pack ]-----------------------------------------------------" >>$REPORT
rep_cmd $REPORT "cat /etc/SuSE-release | grep PATCHLEVEL | awk '{print $3}'"

# Report OES2 version
if [ -e /etc/novell-release ] 
    then
	echo "--[ OES2 Version ]----------------------------------------------------------" >>$REPORT
	rep_cmd $REPORT "cat /etc/novell-release | grep Novell | cut -d'(' -f1"
	echo "--[ OES2 Service Pack ]-----------------------------------------------------" >>$REPORT
	rep_cmd $REPORT "cat /etc/novell-release | grep PATCHLEVEL | awk '{print $3}'"
fi

# Linux Kernel version
echo "--[ Kernel Version ]--------------------------------------------------------" >>$REPORT
rep_cmd $REPORT "uname -r"
echo "--[ Architecture ]----------------------------------------------------------" >>$REPORT
rep_cmd $REPORT "uname -i"

# IP Address and Hostname
echo "--[ IP Address of server ]--------------------------------------------------" >>$REPORT
rep_cmd $REPORT 'cat /etc/hosts | grep $HOST | cut -f1 -d " "'
echo "--[ Hostname ]--------------------------------------------------------------" >>$REPORT
rep_cmd $REPORT "hostname -f"

# Report SLES uptime statistics
echo "--[ Server Uptime ]---------------------------------------------------------" >>$REPORT
rep_cmd $REPORT "/usr/bin/uptime"

# Report Memory usage
echo "--[ Memory Usage ]----------------------------------------------------------" >>$REPORT
rep_cmd $REPORT "/usr/bin/free -mot"

# Report Virtual Memory Statistics
echo "--[ Virtual Memory Statistics ]---------------------------------------------" >>$REPORT
if [ -e /usr/bin/vmstat ]
    then
	rep_cmd $REPORT "/usr/bin/vmstat 2 4"
    else
	echo -e "sysstat and the vmstat package is not installed, you must install these packages to see the vmstat report." >>$REPORT
fi

# Report disk space usage
echo "--[ File System Space Usage ]-----------------------------------------------" >>$REPORT
rep_cmd $REPORT "/bin/df -h | grep -w dev"

# Report inode usage
echo "--[ Inode usage on Linux FS ]-----------------------------------------------" >>$REPORT
rep_cmd $REPORT "/bin/df -i | grep -w dev"
...
if [ -e $NDSCFG ]
	...
	rep_cmd $REPORT '/sbin/ncpcon "connections" 2>/dev/null'
	
	echo "--[ NCP Statistics ]--------------------------------------------------------" >>$REPORT
	rep_cmd $REPORT '/sbin/ncpcon "stats" 2>/dev/null'

	echo "--[ NCP Thread usage ]------------------------------------------------------" >>$REPORT
	rep_cmd $REPORT '/sbin/ncpcon "threads" 2>/dev/null'
	...
	$NDSBIN/ndstrace -c "modules" >>$REPORT
	addspace
	sleep 5
