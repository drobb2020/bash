...
# Last updated: Tue Feb 12 12:55:00 2013 
...
if [ $slpdReturnCode == "0" ] 
	...
	if [ -n $EMAIL ]
	    then
		echo -e "Service Location Protocol daemon is not running on server: $HOST" | mail -s "SLP is DOWN" $EMAIL
	fi
	...
	if [ $slpReturnCode2 == "0" ]
		...
		echo -e "The restart of the slpd daemon failed, please investigate $HOST" | mail -s "SLP Restart Failed" $EMAIL
	fi 
fi
